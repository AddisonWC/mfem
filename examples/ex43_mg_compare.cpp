...
int main(int argc, char *argv[])
{
   ...
   args.AddOption(&choice_arg, "-smoother", "--smoother", "all, macro, macro-vertex, or split.");
   ...
   bool success = true;
   std::cout << "refinement,elements,dofs,smoother,damping,steps,iterations,residual,converged,setup_s,solve_s\n"
             << std::setprecision(9);
   ...
   for (int depth = 0; depth <= refinements; depth++)
   {
      ...
      rhs.Randomize(1);
      for (const std::string kind : {"macro", "macro-vertex", "split"})
      {
         ...
         std::vector<std::unique_ptr<Solver>> local, mapped, scaled;
